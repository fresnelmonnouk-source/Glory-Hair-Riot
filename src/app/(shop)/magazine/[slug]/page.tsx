...
export async function generateMetadata(props: PageProps): Promise<Metadata> {
  ...
  if (!article) {
    return { title: 'Article introuvable · Glory Hair RIOT' };
  }
  return {
    title: `${article.title} · Magazine · Glory Hair RIOT`,
...
export default async function ArticlePage(props: PageProps) {
  ...
  return (
    <article className="mx-auto max-w-[760px] px-6 py-12 md:px-11 md:py-16">
      <Link href="/magazine" className="text-sm text-muted transition-colors hover:text-ink">
        ← Retour au magazine
      </Link>
